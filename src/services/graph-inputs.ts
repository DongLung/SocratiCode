...
import { detectionHeadWindow, readFileHead } from "./extensionless.js";
...
/**
 * Raised by {@link GraphInputRecorder.finish} when the build observed one file
 * two irreconcilable ways.
 *
 * The build reads some sources twice — resolver setup builds its namespace,
 * FQCN, module and `class_name` maps before the main loop parses the same
 * files — so a file edited in between leaves resolver output describing one
 * version while nodes, imports and symbols describe another. That graph is
 * internally mixed, and a record of either version would let the next update
 * call it current. Neither half is worth persisting, so the build is abandoned
 * and retried from a fresh scan instead.
 */
export class GraphInputsMovedDuringBuild extends Error {
  /** Project-relative path → what disagreed about it. */
  readonly conflicts: ReadonlyMap<string, string>;

  constructor(conflicts: ReadonlyMap<string, string>) {
    const [first] = [...conflicts];
    super(
      `${conflicts.size} graph input(s) changed while the graph was being built` +
        (first ? ` — ${first[0]}: ${first[1]}` : ""),
    );
    this.name = "GraphInputsMovedDuringBuild";
 ...
 *
 * Repeated observations of one file are consistency-checked, not overwritten:
 * the first stands, and a later one that contradicts it is a conflict rather
 * than a replacement, because the build has by then already used the first.
 */
...
export function createGraphInputRecorder(projectRoot: string): GraphInputRecorder {
  ...
  /**
   * Head-window hash of every file read whole, so a head-read arriving after
   * one can be checked against the same bytes rather than guessed at. Never
   * persisted — the content hash it belongs to already covers it.
   */
  const windowOfRead = new Map<string, string>();
  const conflicts = new Map<string, string>();
  const rel = (absolutePath: string): string =>
    toForwardSlash(path.relative(root, path.resolve(absolutePath))) || ".";
  /** First conflict per path wins, for the same reason first observations do. */
  const conflict = (key: string, detail: string): void => {
    if (!conflicts.has(key)) conflicts.set(key, detail);
  };
  ...
  return {
    read(absolutePath, content) {
      ...
      const alreadyRead = files.get(key);
      if (alreadyRead !== undefined) {
        // Identical repeat: the resolver pass and the build loop saw one file.
        if (alreadyRead !== hash) conflict(key, "read twice, with different content each time");
        return;
      }
      const headSeen = heads.get(key);
      if (headSeen !== undefined) {
        // Discovery head-read it, and now it has been read whole. Valid only if
        // the window detection scored is still the same bytes. The full hash is
        // what gets kept: it subsumes the window — a change inside the window
        // changes the whole-file hash too — where keeping the head would leave
        // the record blind to everything past it.
        if (headSeen !== hashContent(detectionHeadWindow(content))) {
          conflict(key, "its detection head no longer matches the file read after it");
          return;
        }
        heads.delete(key);
      }
      if (unreadable.has(key)) {
        conflict(key, "could not be read, then read");
        return;
      }
      files.set(key, hash);
      windowOfRead.set(key, hashContent(detectionHeadWindow(content)));
      // A whole-file read supersedes a sighting that never opened it.
      presence.delete(key);
    },
    head(absolutePath, headText) {
      const key = rel(absolutePath);
      const hash = hashContent(headText);
      const windowRead = windowOfRead.get(key);
      if (windowRead !== undefined) {
        if (windowRead !== hash) {
          conflict(key, "its detection head no longer matches the file read before it");
        }
        return;
      }
      const headSeen = heads.get(key);
      if (headSeen !== undefined) {
        if (headSeen !== hash) conflict(key, "head-read twice, with different content each time");
        return;
      }
      if (unreadable.has(key)) {
        conflict(key, "could not be read, then head-read");
        return;
      }
      heads.set(key, hash);
    },
    ...
    unreadable(absolutePath) {
      const key = rel(absolutePath);
      // A file the build already read and then could not read is the same mixed
      // snapshot the other way round: whatever was built from those bytes
      // stands on content this record would no longer describe.
      if (files.has(key) || heads.has(key)) {
        conflict(key, "read, then could not be read");
        return;
      }
      unreadable.add(key);
    },
