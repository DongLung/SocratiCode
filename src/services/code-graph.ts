...
export { describeGraphBuilder, describeUnresolvedSymbolEdges, findCircularDependencies, generateMermaidDiagram, getFileDependencies, getGraphStats, isGraphBuilderStale, isImportResolutionLow } from "./graph-analysis.js";
...
/**
 * How many times a build is retried when files move underneath it.
 *
 * Bounded because a tree under continuous change will never produce a
 * consistent snapshot, and an unbounded retry would spin rather than say so.
 */
const MAX_CONSISTENT_BUILD_ATTEMPTS = 3;

/**
 * Build the graph, retrying from a fresh scan while files move underneath it.
 *
 * The build reads some sources twice, so a file edited between those reads
 * yields a graph whose resolver half and node half describe different content.
 * The recorder refuses to seal a record for that, and the answer is a fresh
 * scan rather than a repaired one: nothing about the mixed attempt is worth
 * keeping, and a retry is cheap relative to serving a graph nobody can date.
 *
 * Gives up after {@link MAX_CONSISTENT_BUILD_ATTEMPTS} by rethrowing, which
 * the incremental update reports as a non-fatal graph-build failure and which
 * leaves the previous graph — cached and persisted — exactly where it was.
 */
async function buildConsistentCodeGraph(
  resolvedPath: string,
  opts: RebuildGraphOptions,
  progress: GraphBuildProgress,
): Promise<Awaited<ReturnType<typeof buildCodeGraph>>> {
  let moved: GraphInputsMovedDuringBuild | undefined;
  for (let attempt = 1; attempt <= MAX_CONSISTENT_BUILD_ATTEMPTS; attempt++) {
    // A retry starts from nothing, so the counters must too, or a watcher sees
    // one build's files added to the last one's.
    progress.filesTotal = 0;
    progress.filesProcessed = 0;
    progress.filesSkipped = undefined;
    progress.phase = "scanning files";
    try {
      return await buildCodeGraph(resolvedPath, opts.extraExtensions, progress);
    } catch (err) {
      if (!(err instanceof GraphInputsMovedDuringBuild)) throw err;
      moved = err;
      logger.warn("Files changed while the code graph was being built — retrying from a fresh scan", {
        projectPath: resolvedPath,
        attempt,
        of: MAX_CONSISTENT_BUILD_ATTEMPTS,
        conflicts: [...err.conflicts].map(([file, detail]) => ({ file, detail })).slice(0, 5),
      });
    }
  }
  throw moved;
