...
describe("code-index effective profile compatibility", () => {
  ...
  it("hands the caller's own extension set to the rebuild decision", async () => {
    // The set reaching the decision has to be the set the index was built with,
    // or a graph built with per-call extensions gets compared under the
    // defaults and the leaf nodes they admit read as a configuration mismatch.
    const indexer = await loadIndexer();
    const { legacyIndexProfile } = await import("../../src/services/index-profile.js");
    const { rebuildGraph, shouldRebuildGraph } = await import("../../src/services/code-graph.js");
    const project = await createProject("notes.txt", "new changed content");
    ...
    const extras = new Set([".rst", ".adoc"]);
    vi.mocked(shouldRebuildGraph).mockClear();
    vi.mocked(rebuildGraph).mockClear();
    await indexer.updateProjectIndex(project, undefined, extras);

    expect(vi.mocked(shouldRebuildGraph)).toHaveBeenCalledTimes(1);
    // The same object, not merely an equal one: a copy would mean something in
    // between decided what the graph's extensions are.
    expect(vi.mocked(shouldRebuildGraph).mock.calls[0][2]).toBe(extras);
    expect(vi.mocked(rebuildGraph).mock.calls[0][1]).toMatchObject({ extraExtensions: extras });
  });
