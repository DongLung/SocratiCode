...
import { readFileHead } from "../../src/services/extensionless.js";
import {
...
describe("graph input invalidation", () => {
  ...
  describe("one file observed twice", () => {
    // The build reads some sources twice — resolver setup builds its namespace,
    // FQCN and module maps before the main loop parses the same files. A file
    // edited in between leaves the two halves describing different content, and
    // a record of either half would let the next update call that graph current.
    const recorderFor = () => createGraphInputRecorder("/project");
    const finish = (recorder: ReturnType<typeof createGraphInputRecorder>) =>
      recorder.finish(EXTRA_EXTENSIONS, "cafebabecafebabe");

    it("accepts an identical repeated read", () => {
      // The ordinary case, and the one that must not start failing: two passes
      // over an unchanged file.
      const recorder = recorderFor();
      recorder.read("/project/src/a.ts", "export const a = 1;\n");
      recorder.read("/project/src/a.ts", "export const a = 1;\n");

      expect(finish(recorder).files["src/a.ts"]).toBe(hashContent("export const a = 1;\n"));
    });

    it("refuses to seal a record for a file read twice with different content", () => {
      const recorder = recorderFor();
      recorder.read("/project/src/a.ts", "export const a = 1;\n");
      recorder.read("/project/src/a.ts", "export const a = 2;\n");

      expect(() => finish(recorder)).toThrow(GraphInputsMovedDuringBuild);
      try {
        finish(recorder);
        expect.unreachable("finish should have thrown");
      } catch (err) {
        expect((err as GraphInputsMovedDuringBuild).conflicts.get("src/a.ts")).toBe(
          "read twice, with different content each time",
        );
      }
    });

    it("refuses to seal a record for a file read and then unreadable", () => {
      // The same mixed snapshot the other way round: something was built from
      // bytes this record would no longer describe.
      const recorder = recorderFor();
      recorder.read("/project/src/a.ts", "export const a = 1;\n");
      recorder.unreadable("/project/src/a.ts");

      expect(() => finish(recorder)).toThrow(GraphInputsMovedDuringBuild);
    });

    it("accepts the extensionless head-to-full transition when the window is unchanged", () => {
      // Discovery scores a head; the loop reads the file whole. Valid as long as
      // the bytes detection scored are still those bytes — tested at the exact
      // window boundary, with content continuing past it.
      const window = "#!/bin/sh\n".padEnd(DETECT_HEAD_BYTES, "x");
      const full = `${window}echo past the window\n`;
      expect(Buffer.byteLength(window, "utf-8")).toBe(DETECT_HEAD_BYTES);

      const recorder = recorderFor();
      recorder.head("/project/bin/tool", window);
      recorder.read("/project/bin/tool", full);

      const record = finish(recorder);
      // The whole-file hash is what is kept: it covers the window and the rest,
      // where the head alone would leave everything past byte 8192 unwatched.
      expect(record.files["bin/tool"]).toBe(hashContent(full));
      expect(record.heads["bin/tool"]).toBeUndefined();
    });

    it("accepts the transition for content whose bytes decode lossily", async () => {
      // An invalid UTF-8 byte decodes to U+FFFD and re-encodes to three, so a
      // head taken from raw bytes and a window taken from decoded content end
      // in different places unless both go through the canonical window. They
      // did not, and an unchanged file of this shape then conflicted on every
      // attempt and exhausted the build's retries.
      // Its own directory, not the shared fixture: a file appearing there is an
      // addition, and every later decision in this file would rebuild for it.
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "socraticode-lossy-"));
      const file = path.join(dir, "lossy");
      try {
        fs.writeFileSync(
          file,
          Buffer.concat([
        ...
        const recorder = createGraphInputRecorder(dir);
        recorder.head(file, await readFileHead(file));
        recorder.read(file, fs.readFileSync(file, "utf-8"));

        expect(() => finish(recorder)).not.toThrow();
      } finally {
        fs.rmSync(dir, { recursive: true, force: true });
      }
    });

    it("refuses to seal a record when the head no longer matches the file read after it", () => {
      const recorder = recorderFor();
      recorder.head("/project/bin/tool", "#!/bin/sh\n");
      recorder.read("/project/bin/tool", "#!/usr/bin/env python\nprint(1)\n");

      expect(() => finish(recorder)).toThrow(GraphInputsMovedDuringBuild);
    });

    it("keeps a whole-file read over a later sighting that never opened the file", () => {
      // Legitimate, not a conflict: a resolver reads a source with no size limit
      // of its own, and the build loop then excludes it as too large. Both
      // observed the same content, and the hash is the stronger record.
      const recorder = recorderFor();
      recorder.read("/project/src/big.cs", "namespace A {}\n");
      recorder.present("/project/src/big.cs", 2_000_000);

      const record = finish(recorder);
      expect(record.files["src/big.cs"]).toBe(hashContent("namespace A {}\n"));
      expect(record.presence["src/big.cs"]).toBeUndefined();
    });
  });
